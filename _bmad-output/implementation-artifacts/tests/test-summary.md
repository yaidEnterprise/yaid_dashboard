...
- [x] tests/unit/story-5-9/mistral-ocr-provider-selection.test.mjs - Story 5.9 (dev) contratos estruturais: `MistralOcrProvider` existe/implementa `OcrProvider`, único arquivo em `src/` que importa `@mistralai/mistralai` (guard codebase-wide), uso de `documentAnnotationFormat`/`jsonSchema`/`schemaDefinition` (nomes reais do SDK, não `json_schema`/`schema`), `JSON.parse` incondicional de `documentAnnotation`, validação de CPF (11 dígitos)/birthDate (`YYYY-MM-DD`)/name, ausência de `console.*`, `timeoutMs` explícito, `ApiOcrProvider.ts` removido, `environments.ts` sem `OCR_API_URL`/`OCR_API_KEY`, `MISTRAL_API_KEY` obrigatória em produção com getter que lança, `getOcrProvider()` ramifica só por `STAGE === Stage.TEST` sem fallback por presença de config, compilação TypeScript limpa. Inclui 3 testes de regressão dos patches do code review (MIME não reconhecido lança, data de calendário inválida rejeitada, `MIN_BIRTH_YEAR`)
- [x] tests/unit/story-5-9/mistral-ocr-provider.dynamic.test.ts - Story 5.9 (QA) **teste dinâmico/comportamental** — instancia `MistralOcrProvider` real e mocka apenas `client.ocr.process` (único ponto de I/O de rede da classe) via `node:test`'s `mock.method`, verificando o comportamento efetivo de `processDocument` em runtime: AC#1 (annotation válida → `{name, cpf, birthDate}` normalizado, request contém `documentAnnotationFormat` e uma data URI `image/png`), AC#2 (annotation ausente/`null` → rejeitado), AC#3 (CPF com contagem errada, campo `null`, `birthDate` futuro → rejeitados), JSON malformado em `documentAnnotation` → rejeitado sem vazar `SyntaxError` cru, `name` < 3 chars após trim → rejeitado, MIME JPEG detectado corretamente na data URI. Mais 3 testes de regressão dos patches do code review: data de calendário inválida (29/02 em ano não bissexto) rejeitada em vez de sofrer rollover silencioso do `Date`, ano de nascimento abaixo de `MIN_BIRTH_YEAR` rejeitado, formato de imagem não reconhecido rejeitado **antes** de chamar a API (mock nunca invocado — prova que o guard roda primeiro). Verificado por mutação: desabilitar a checagem `MIN_BIRTH_YEAR` ou o guard de rollover de calendário faz os testes correspondentes falharem, confirmando que exercitam o código real (não apenas regex sobre o source). Fecha a lacuna "suíte 100% estática" registrada no code review desta story. Executado via `tsx --test` (mesmo padrão das Stories 5.8/7.2/7.3/9.1/11.1)
...
### Story 5.9 — OCR Estruturado via Mistral Document AI (QA layer sobre os 3 patches de review)

- Contexto: Story 5.9 substituiu `ApiOcrProvider` (OCR genérico + regex de extração) por `MistralOcrProvider` (Mistral Document AI com `document_annotation_format` estruturado), e reescreveu `getOcrProvider()` para selecionar o provider só por `STAGE`, sem fallback silencioso por ausência de configuração. Code review (Blind Hunter + Edge Case Hunter + Acceptance Auditor) confirmou as 11 ACs satisfeitas e aplicou 3 patches de hardening. Esta etapa de QA adiciona cobertura dinâmica/comportamental que exercita `processDocument` em runtime (mockando só o SDK), fechando o gap "suíte 100% estática" que os próprios revisores apontaram.
- Acceptance criteria (11/11 cobertos — dev + QA):
  - AC#1 (extração estruturada, zero regex, VC emitida): coberto estruturalmente (dev) + dinamicamente (`processDocument` retorna `{name, cpf, birthDate}` a partir de uma `documentAnnotation` real mockada, request contém `documentAnnotationFormat`)
  - AC#2 (documento sem campos → 422): coberto dinamicamente (`documentAnnotation: null` → rejeitado)
  - AC#3 (saída inválida da Mistral → 422, nunca aceita sem validação): coberto dinamicamente (CPF com contagem errada, campo `null`, `birthDate` futuro, calendário inválido, ano implausível — todos rejeitados)
  - AC#4 (nada de log, mesmo em erro): coberto estruturalmente (dev — `assert.doesNotMatch(src, /console\./)`)
  - AC#5/AC#6 (matriz `STAGE` → provider, sem fallback): coberto estruturalmente (dev)
  - AC#7 (SDK confinado a um único arquivo): coberto estruturalmente (dev — guard codebase-wide)
  - AC#8 (`ApiOcrProvider` removido): coberto estruturalmente (dev)
  - AC#9 (12 nomes canônicos em CI/CD/docs/testes 11.8): coberto estruturalmente (dev + testes de `story-11-8` atualizados)
  - AC#10 (Secret cadastrado antes do deploy): ação operacional, documentada nos Dev Notes — não testável em código
  - AC#11 (`npm run test` passa integralmente): coberto pela execução completa da suíte
- Patches de review cobertos por teste NOVO nesta etapa de QA (todos verificados por mutação — desabilitar o guard correspondente faz o teste falhar):
  1. `detectMimeType` lança para formato de imagem não reconhecido em vez de assumir PNG — teste dinâmico prova que o mock do SDK **nunca é chamado** quando a imagem não bate PNG/JPEG (guard roda antes de qualquer I/O)
  2. `isValidBirthDate` rejeita datas de calendário inválidas (rollover do `Date` do JS) — teste dinâmico com `"2023-02-29"` (não bissexto)
  3. `isValidBirthDate` rejeita anos abaixo de `MIN_BIRTH_YEAR` (1900) — teste dinâmico com `"1800-01-01"` (ano escolhido para não colidir com a regra legada de dois dígitos do `Date.UTC` para anos 0-99)
- Caminhos críticos: 35/35 testes passando na story (24 dev estruturais + 11 QA dinâmicos)
  - Diferencial desta etapa de QA: mocka só `client.ocr.process` (o único ponto de I/O de rede da classe) via `node:test`'s `mock.method` nativo — sem biblioteca de mock externa, sem chamada de rede real à API da Mistral
  - Fecha o "deferred" registrado em `deferred-work.md` pelo code review desta própria story: *"Suíte de testes é 100% estática... nenhum teste instancia MistralOcrProvider com o SDK mockado para exercitar processDocument em runtime"*

#### Validation

- `npm run test:story:5.9`: **passed** — 35/35 (24 dev estruturais via `node --test` + 11 QA dinâmicos via `tsx --test`)
- `npm test` (suíte completa) DEPOIS desta etapa de QA: **passed** — 974 estáticos + 74 dinâmicos, 1048/1048 passando, 0 falhas (antes da etapa de QA: 974 estáticos + 63 dinâmicos — delta exato de +11 dinâmicos, os testes novos desta etapa)

#### Notes

- Nenhum arquivo de produção alterado nesta etapa — só o teste dinâmico novo (`mistral-ocr-provider.dynamic.test.ts`), o ajuste de uma asserção desatualizada no teste estrutural do dev (regex antiga do `birthDate` após o patch de calendário), o script `test:story:5.9` em `package.json`, e este `test-summary.md`
- Nenhum defeito de implementação genuíno encontrado durante a execução real do provider mockado: os 3 patches do code review se comportam exatamente como descrito no Change Log da story sob as fixtures testadas
- Decisão de mock: `client.ocr.process` é acessado via `(provider as unknown as {...}).client.ocr.process` e trocado com `mock.method` — evita depender de qualquer capacidade de mock de módulo do `tsx`/ESM e funciona porque `client` é uma propriedade de instância real (não um import estático) que o construtor de `MistralOcrProvider` já materializa com uma chave de API fake (nenhuma chamada de rede acontece, pois o método é substituído antes de qualquer `processDocument()`)
- Refino 422 vs 502 (distinguir "documento ilegível" de "provider indisponível") permanece deliberadamente fora de escopo, conforme o Sprint Change Proposal 2026-08-19 §4.5 e o próprio code review desta story — nenhum teste foi escrito para esse comportamento porque ele não existe no código
- [x] tests/unit/story-7-5/review-manual-em-apps-homologacao.test.mjs - Story 7.5 (dev) contratos estruturais: `ReviewProofRequestSchema` (enum `approve`/`reject`), ordem dos guards no use case (404 ownership -> 403 ambiente -> 422 status terminal, verificado por posição de índice no source), `updateStatus` antes do disparo do webhook, webhook fire-and-forget (`.catch()`, nunca `await`), rota `POST` wired ao presenter/`handleHttpError`, presenter compõe `ProofRequestRepository`+`DeliverWebhookUseCase`, controller valida body com Zod, regra dedicada de session-auth para `POST .../review` no middleware, client store `reviewProofRequest` (POST + `{decision}` + `/review`), página (`canReview`, botões condicionais, texto do diálogo de confirmação, `toast.success`/`toast.error`, atualização local de `data` sem refetch)
- [x] tests/unit/story-7-5/review-proof-request.dynamic.test.ts - Story 7.5 (dev) **teste dinâmico/comportamental** — instancia `ReviewProofRequestUseCase`/`ReviewProofRequestController` reais com `ProofRequestRepository`/`DeliverWebhookUseCase` fake (padrão de `verify-presentation-vc-jwt.dynamic.test.ts` da Story 9.2): approve/reject efetivamente transicionam o status e persistem via `updateStatus`, o webhook recebe `proofRequestId`/`status`/`proofType` reais, falha na entrega do webhook não propaga para o caller (fire-and-forget), guard 403 para `environment` `prod` e `dev` legado, guard 422 para os 3 status terminais (`approved`/`rejected`/`expired`), guard 404 tanto para request inexistente quanto para company diferente (nunca 403, anti-enumeration), e o controller rejeita `decision` inválido com `ZodError` antes de chegar ao use case

### Story 7.5 — Review Manual (Aprovar/Reprovar) em Apps de Homologação
- Acceptance criteria: 6/6 cobertos
  - AC#1 (approve/reject transiciona status, grava `updated_at`, dispara webhook real): coberto pelos testes dinâmicos de transição approve/reject e pelas asserções de `statusUpdates`/`webhook.calls`
  - AC#2 (app `prod` -> 403, defesa em profundidade): coberto pelo teste dinâmico parametrizado para `prod` e `dev` (legado) — ambos rejeitados
  - AC#3 (status terminal -> 422, sem re-transição): coberto pelo teste dinâmico parametrizado para os 3 status terminais (`approved`/`rejected`/`expired`)
  - AC#4 (`requestId` de outra company -> 404, sem enumeration): coberto pelos testes dinâmicos de not-found e company-mismatch, ambos afirmando `NotFoundError` e explicitamente **não** `ForbiddenError`
  - AC#5 (botões Aprovar/Reprovar + confirmação + `toast.success` + atualização local): coberto estruturalmente — `canReview`, gating dos botões, texto exato do diálogo ("Esta ação envia o webhook real..."), `toast.success` com as duas variantes de cópia, `setData` local sem refetch
  - AC#6 (app `prod` ou status terminal -> botões ausentes na UI): coberto pela mesma condição `canReview` verificada no AC#5 (é a mesma expressão booleana que gate os botões)
- Caminhos críticos: 29/29 testes passando (16 estruturais + 13 comportamentais)
  - Guard order (404->403->422) verificado tanto por índice de posição no source (estrutural) quanto por comportamento real (dinâmico)
  - Webhook fire-and-forget: verificado que `updateStatus` roda antes do disparo do webhook, que o webhook nunca é `await`ado diretamente, e que uma falha na entrega não rejeita o use case
  - Middleware: regra dedicada de `isSessionAuthApiRoute` para `POST .../review` confirmada — sem essa regra a rota ficaria sem autenticação de sessão
  - Controller: validação de `decision` via Zod antes do use case, cobrindo tanto o caminho inválido (`ZodError`) quanto o válido

#### Validation
- `npm run test:story:7.5`: **passed** — 29/29 (16 `.test.mjs` + 13 `.dynamic.test.ts`), reconfirmado após os 4 patches do code review
- `npm test` (suite completa): **passed** — 957/966 (9 falhas pré-existentes em `tests/unit/story-11-8/env-var-sync-guards-contract.test.mjs`, ambiente sem `bash`/`aws` stub totalmente funcional nesta sessão — mesmo arquivo que a própria Story 11.8 documenta como "0 falhas" em outro ambiente; não relacionado a esta story, confirmado via `git status --short` sem alterações nesse arquivo)
- `npx tsc --noEmit`: **passed** — limpo, zero erros
- `npm run lint`: **passed** — 18 problemas (6 erros/12 warnings), mesma contagem pré-existente documentada desde a Story 7.3; nenhum novo nos arquivos criados/modificados por esta story

#### Notes
- Segue a convenção dupla já estabelecida pelas Stories 7.3/9.2: `.test.mjs` estrutural (regex sobre source, cobre rota/middleware/presenter/controller/página) + `.dynamic.test.ts` comportamental (`tsx --test`, fakes reais implementando as interfaces do domínio, cobre a lógica de negócio do use case/controller)
- Guards de ambiente/status/ownership têm cobertura comportamental completa; rota HTTP, middleware e página só têm cobertura estrutural — consistente com o padrão de todas as stories anteriores que tocam essas mesmas camadas (não há `jsdom`/`@testing-library` no projeto, decisão pré-existente registrada desde a Story 4.2)
- 9 itens deferidos do code review para `deferred-work.md` — majoritariamente limitações estruturais pré-existentes replicadas fielmente de precedentes já shippados (`cancel_proof_session_usecase.ts` para a race condition/`updatedAt`; `DisableConfirmDialog` para a ausência de focus trap e o padrão de `onCancel` inline; a rota GET irmã para a non-null assertion e a ausência de validação de formato de `requestId`), não regressões introduzidas por esta story
- 4 patches do code review aplicados e cobertos pela suíte existente (nenhum teste novo foi necessário especificamente para os patches, já que as asserções estruturais/comportamentais existentes continuam válidas após as correções): regex do middleware tolerante a barra final, guard `!loading` no diálogo, mensagem de erro unificada em inglês, resync de `data` após erro de review

- [x] tests/unit/story-10-1/key-centralization.test.mjs - Story 10.1 (dev) contratos estruturais: `TEST_ENV` carrega `ISSUER_PRIVATE_KEY`/`WEBHOOK_SIGNING_PRIVATE_KEY` como os hex de 32 bytes prontos (`...0001`/`...0002`), `BLOCKCHAIN_WALLET_PRIVATE_KEY` inalterado (fora de escopo), ausência do remendo de placeholder nos quatro consumidores (`issue_credential_usecase.ts`, `verify_presentation_usecase.ts`, `Ed25519WebhookSigner.ts`, `get_webhook_public_key_usecase.ts`), remoção do parâmetro `stage`/import `Stage` no use case e no presenter do webhook-public-key, preservação do `HEX_PRIVATE_KEY_PATTERN` (não é um dos 4 pontos removidos), compilação TypeScript limpa
- [x] tests/unit/story-10-1/environments-key-centralization.dynamic.test.ts - Story 10.1 (dev) **teste dinâmico/comportamental** — chama `Environments.getEnvs()` real sob `STAGE=TEST` e confirma que `.ISSUER_PRIVATE_KEY`/`.WEBHOOK_SIGNING_PRIVATE_KEY` retornam exatamente os mesmos valores hex que os quatro remendos removidos produziam, travando em runtime a garantia da AC#1 ("nenhuma assinatura muda")
- [x] tests/unit/story-10-1/key-signing-roundtrip.dynamic.test.ts - Story 10.1 (QA) **teste dinâmico/comportamental** — instancia `Ed25519WebhookSigner`/`GetWebhookPublicKeyUseCase` reais (construtor de um único argumento, pós-remoção do parâmetro `stage`) com o valor hex exato de `TEST_ENV` e confirma o round-trip Ed25519 de ponta a ponta (assinatura válida contra a public key derivada; payload adulterado falha a verificação). Fecha a lacuna: os testes anteriores (estáticos + `story-6-2`) já provavam a ausência do remendo e faziam round-trip com `ed.signAsync`/`ed.verifyAsync` diretos, mas nenhum exercitava as classes de produção simplificadas de fato construídas com um único argumento

### Story 10.1 — Centralização de Chaves de Teste no `environments.ts`
- Acceptance criteria: 5/5 cobertos
  - AC#1 (`TEST_ENV` carrega os hex prontos, nenhuma assinatura muda): coberto estaticamente (`key-centralization.test.mjs`) e em runtime por dois testes dinâmicos — `environments-key-centralization.dynamic.test.ts` confirma o getter de `Environments`, `key-signing-roundtrip.dynamic.test.ts` confirma que o par de chaves resultante assina/verifica corretamente através das classes de produção reais
  - AC#2 (os quatro pontos de substituição removidos, consumidor usa o valor recebido sem inspecionar): coberto por 4 asserções estruturais dedicadas (uma por arquivo) mais a confirmação de que `HEX_PRIVATE_KEY_PATTERN` (guard de formato do próprio arquivo, não um dos 4 pontos) permanece intacto
  - AC#3 (testes pré-existentes reescritos, asserções do valor `...0002` inalteradas): coberto por regressão — `webhook-delivery.test.mjs`/`webhook-public-key.test.mjs` reescritos e reconfirmados passando; os blocos de round-trip/response-shape/hex-validation de `story-6-2` que usam `...0002` diretamente permanecem intocados e verdes
  - AC#4 (suíte completa passa integralmente): coberto por regressão de suíte completa, incluindo o gap descoberto (`story-9-1/vc-jwt-issuance.test.mjs`) e um segundo gap descoberto durante a implementação (`story-1-1/restructure.test.mjs`)
  - AC#5 (novo consumidor recebe valor pronto): coberto implicitamente pelas ACs #1/#2 — não há mais nenhum ponto no código que conheça ou precise conhecer o placeholder
- Caminhos críticos: 16/16 testes passando nesta story (13 estáticos + 3 dinâmicos: 2 criados no dev-story + 1 criado nesta etapa de QA)
  - Round-trip Ed25519 real através das classes de produção simplificadas (`Ed25519WebhookSigner`/`GetWebhookPublicKeyUseCase` com construtor de um argumento), não apenas via chamadas diretas a `ed.signAsync`/`ed.verifyAsync`
  - `Environments.getEnvs()` real sob `STAGE=TEST` confirmado retornando os valores hex exatos anteriormente produzidos pelos remendos
  - Regressão das 3 suítes pré-existentes reescritas na Task 6 do dev-story, mais a suíte da Story 1.1 corrigida durante a mesma etapa

#### Validation
- `npm run test:story:10.1`: **passed** — 16/16 (13 `.test.mjs` via `node --test` + 3 `.dynamic.test.ts` via `tsx --test`)
- `npm test` (suíte completa) APÓS esta etapa de QA: **passed** — 966/975 estáticos + 79/79 dinâmicos (antes desta etapa de QA: 966/975 + 77/77 — delta exato de +2 dinâmicos, os testes novos desta etapa)
- As 9 falhas estáticas restantes são pré-existentes em `tests/unit/story-11-8/env-var-sync-guards-contract.test.mjs` (testes de bash real do pipeline Amplify) — confirmado via `git stash` durante o dev-story desta mesma story que já falhavam **antes** do diff ser aplicado neste ambiente; sem relação com `environments.ts` ou chaves
- `npx eslint` no novo arquivo de teste desta etapa: 0 findings

#### Notes
- Nenhum arquivo de produção alterado nesta etapa de QA — só o teste novo (`tests/unit/story-10-1/key-signing-roundtrip.dynamic.test.ts`) e este `test-summary.md`
- Nenhum defeito de implementação encontrado durante esta etapa: o comportamento de assinatura/verificação através das classes de produção simplificadas é idêntico ao anterior ao remendo, confirmando a AC#1 por execução real
- Decisão desta etapa: não duplicar cobertura dinâmica para `IssueCredentialUseCase`/`VerifyPresentationUseCase` com o hex real do issuer (`...0001`) — o teste dinâmico da Story 9.1 (`issue-credential-usecase.dynamic.test.ts`) já exercita exatamente a mesma linha de código pós-refatoração (`hexToBytes(this.issuerPrivateKey)`) com uma chave de 32 bytes estruturalmente idêntica (`"22".repeat(32)`); o valor literal da chave não muda a lógica exercida, então um segundo teste só com o hex diferente seria redundante

- [x] tests/unit/story-10-2/key-format-validation.test.mjs - Story 10.2 (dev) contratos estruturais: os 4 campos de chave em `envSchema` viram `z.string().optional()` simples (sem `.min(1)`), import de `ethers` e `HEX_PRIVATE_KEY_PATTERN` presentes, `superRefine` valida formato hex das 3 chaves privadas e `ethers.isAddress` do endereço do contrato com mensagens acionáveis, a checagem de placeholder constrói o conjunto `knownTestValues` a partir de `productionRequiredEnvNames` (não compara só campo-a-campo), as checagens novas usam guard `!== undefined` (não truthy — review patch), ordenação crítica confirmada (checagens novas antes do early-return de `PROD`/`HOMOLOG`), compilação TypeScript limpa
- [x] tests/unit/story-10-2/envSchema-key-validation.dynamic.test.ts - Story 10.2 (dev + review patches) **teste dinâmico/comportamental** — instancia `new Environments().loadEnvs()` real (bypassando o cache do singleton) sob `withEnv()` para cada cenário: formato malformado (typo/comprimento errado/não-hex) nas 3 chaves privadas + endereço do contrato, todos nomeando a variável certa na mensagem específica de validação; valores válidos não lançam em `PROD`; valor de placeholder exato do `TEST_ENV` rejeitado em `PROD`/`HOMOLOG`/`DEV`/`DOTENV` (prova direta de que não é gated por stage); **detecção cross-field** — um valor de placeholder vazado sob o nome de outro campo (ex.: chave do issuer copiada para `WEBHOOK_SIGNING_PRIVATE_KEY`, ou para `BLOCKCHAIN_WALLET_PRIVATE_KEY`) também é rejeitado; string vazia definida explicitamente falha (não é tratada como ausência); `STAGE=TEST` aceita `TEST_ENV` sem rodar a validação nova; `DEV` sem nenhuma das 4 chaves continua tolerado; regressão da checagem de presença pré-existente em `PROD`

### Story 10.2 — Validação de Formato de Chaves no Boot
- Acceptance criteria: 6/6 cobertos
  - AC#1 (ISSUER_PRIVATE_KEY/WEBHOOK_SIGNING_PRIVATE_KEY exigem 64 hex chars, falha nomeando a variável): coberto por 4 casos de formato malformado (typo, comprimento errado, caracteres não-hex) mais o caso positivo com valores válidos
  - AC#2 (BLOCKCHAIN_WALLET_PRIVATE_KEY segue a mesma regra hex; BLOCKCHAIN_CONTRACT_ADDRESS via ethers.isAddress reaproveitado de EthersBlockchainClient): coberto por teste dedicado para cada campo, mais confirmação estrutural de que o mesmo `ethers.isAddress` já usado em `EthersBlockchainClient.ts:33` é reaproveitado, não reimplementado
  - AC#3 (valor de TEST_ENV rejeitado fora do stage TEST, incluindo DOTENV/DEV): coberto por 4 testes cobrindo os 4 stages não-TEST (PROD/HOMOLOG/DEV/DOTENV) mais 2 testes de detecção cross-field (achados e corrigidos durante o code review — a checagem original só comparava cada campo contra seu próprio valor de TEST_ENV)
  - AC#4 (stage TEST aceita TEST_ENV normalmente): coberto por teste que confirma `loadEnvs()` não lança e retorna os valores exatos de TEST_ENV sem rodar nenhuma validação nova
  - AC#5 (DOTENV/DEV sem as chaves definidas continua tolerado): coberto por teste dedicado, mais o teste de regressão da checagem de presença em PROD
  - AC#6 (suíte completa passa integralmente): coberto por regressão de suíte completa após a implementação e novamente após os 5 patches do code review
- Caminhos críticos: 25/25 testes passando nesta story (10 estáticos + 15 dinâmicos — nenhum teste adicional de QA necessário, cobertura já madura após os patches do code review)
  - Ordenação crítica do `superRefine`: as checagens novas (formato, endereço, placeholder) rodam **antes** do early-return gated por `PROD`/`HOMOLOG` — testado tanto estruturalmente (posição no source) quanto comportamentalmente (`STAGE=DEV` + placeholder → lança)
  - 2 bugs reais encontrados e corrigidos no code review, ambos com teste de regressão: string vazia definida explicitamente escapava de toda validação nova (guard truthy → `!== undefined`); detecção de placeholder só comparava campo-a-campo, deixando passar um valor vazado sob o nome errado (comparação campo-a-campo → conjunto completo de valores conhecidos)

#### Validation
- `npm run test:story:10.2`: **passed** — 25/25 (10 `.test.mjs` + 15 `.dynamic.test.ts`), reconfirmado após os 5 patches do code review
- `npm test` (suíte completa): **passed** — 976/985 estáticos (9 falhas pré-existentes em `tests/unit/story-11-8/env-var-sync-guards-contract.test.mjs`, já confirmadas não relacionadas via `git stash` durante a Story 10.1) + 94/94 dinâmicos
- `npx eslint src/shared/environments.ts tests/unit/story-10-2/*.mjs tests/unit/story-10-2/*.ts tests/unit/story-1-1/restructure.test.mjs`: **passed** — 0 findings
- `STAGE=TEST npx tsc --noEmit`: **passed** — limpo, sem erros

#### Notes
- Nenhum teste novo de QA foi necessário — a cobertura do dev-story mais os 5 patches aplicados durante o code review (que já vieram com teste de regressão dedicado cada um) já satisfazem integralmente as 6 ACs. Segue o mesmo padrão da Story 7.5 (cobertura de dev já madura, QA só documenta)
- 10 itens deferidos do code review para `deferred-work.md` — nenhum é um defeito de implementação desta story: são decisões de design já documentadas nas Dev Notes (formato de `BLOCKCHAIN_WALLET_PRIVATE_KEY` sem `0x`), fraquezas herdadas de um precedente que a própria AC manda reaproveitar (`ethers.isAddress` aceitando formatos ICAP/ENS), padrões sistêmicos pré-existentes do projeto (teste `tsc --noEmit` com exclusão de `lucide-react`), ou um item acoplado a outra story (`.env.local.example`/Story 11.8) que precisa de tratamento dedicado
- 7 itens deferidos do code review para `deferred-work.md` (registrados durante a etapa de review, não nesta etapa de QA) — nenhum é um defeito de implementação desta story; são lacunas de validação de formato explicitamente atribuídas à Story 10.2 (`backlog`, depende desta) ou padrões pré-existentes replicados de precedentes já shippados
