...
## Deferred from: code review of spec-github-oidc-deploy-auth (2026-09-02)

- **Guards `STATIC_CREDENTIAL_INPUTS` são estruturais, não full-text** — `tests/unit/story-11-5/workflow-job-deploy-amplify.test.mjs` verifica a ausência de `aws-access-key-id`/`aws-secret-access-key`/`aws-session-token` só no bloco `inputs:` do composite e no `with:` do step que chama o composite em `production.yml`. Um valor perdido em um `run:`/`env:` de outro step ou job não seria pego por esse guard. Não é um problema hoje (grep manual + `npm test` confirmam ausência total no repo), mas um guard `grep`-based sobre o texto bruto dos dois arquivos seria mais robusto contra regressão futura. Candidato a story de hardening. [`tests/unit/story-11-5/workflow-job-deploy-amplify.test.mjs`]
